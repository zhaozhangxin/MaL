...
% Status     n*1           The censoring status; (Its element equals to 1 and 0 for the uncensored and censored sample, respectively))
% X             n*m_x      Original mono-modality feature matrix drived from gene expression data or histopathological data
...
sigma_1=1e-4;              %The positive scalar of constraint U=Y*Psi
sigma_2=1e-6;              %The positive scalar of constraint Y-J
...
%% Procedure
% Calculate similarity matrix S_Z and Laplacian matrix L
sigma=1;
[m_z,n_z]=size(Z);
[m_x,n_x]=size(X);
...
% Calculate the feature matrix of the reference points in the original feature space: R_X, R_Z
R_X=calR(X,C);
R_Z=calR(Z,C);

% Calculate the weight coefficient matrix determined by the Gaussian distance: H_X, H_Z
...
% Calculate the feature matrix of the reference points in the latent representations: R_Y,R_U (initialization)
R_Y=calR(Y,C); 
R_U=calR(U,C);

% Calculate the Euclidean distance matrix between samples and reference points within latent feature subspace: D_Y,D_U (initialization)
D_Y=calD(Y,R_Y);
D_U=calD(U,R_U);
...
for iter=1:iter_times    
    ...
    V_1_new=sigma_1*(U-Y*Psi)+V_1;
    V_2_new=sigma_2*(Y-J)+V_2;
    ...
    % Update all variables after one iteration
    Y=Y_new;
    Y=(mapminmax(Y',0,1))';
    U=U_new;
    U=(mapminmax(U',0,1))';
    ...
    % Calculate the feature matrix of the reference points in the latent representations: R_Y,R_U (for the next iteration)
    R_Y=calR(Y,C); 
    R_U=calR(U,C);

    % Calculate the Euclidean distance matrix between samples and reference points within latent feature subspace: D_Y,D_U (for the next iteration)
    D_Y=calD(Y,R_Y);
    D_U=calD(U,R_U);
...
function D=calD(X,R)
% X is the input feature matrix; R is the reference points matrix
[m1,~]=size(X);
[m2,~]=size(R);
    for j=1:m1
        for k=1:m2
...
% sigma is the standard deviation of the Gaussian function; X is the input feature matrix; R is the reference points matrix
[m1,~]=size(X);
[m2,~]=size(R);
    for j=1:m1
        for k=1:m2
